...
// ── Aliyun OSS ──────────────────────────────────────────────────────────────────
//
// Deliberately SDK-free: OSS object PUT/DELETE is signed with the documented
// header signature (V1), which keeps this plugin free of a heavy dependency and
// free of a network install step.
...
function ossEndpoint(ossConfig) {
  ...
  return { region, bucket, host: `${bucket}.${region}.aliyuncs.com` };
}
...
async function ossRequest(ossConfig, method, key, body) {
  ...
  const { bucket, host } = ossEndpoint(ossConfig);
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `OSS ${method} 失败 ${response.status} ${response.statusText}` +
...
function localRoot(ossConfig) {
  return ossConfig.localDir?.trim() ? ossConfig.localDir.trim() : join(dshHome(), 'repo-snapshots');
}
 ...
 */
function registerRoutes(ctx, webServer, state, cordisConfig, logger) {
  const disposers = [];
...
export function apply(ctx, cordisConfig) {
  ...
  // Routes are optional: a headless run has no webServer.
  const webServer = ctx.get('webServer');
  if (webServer && typeof webServer.register === 'function') {
    const disposers = registerRoutes(ctx, webServer, state, cordisConfig, logger);
    ctx.effect(() => () => {
    ...
    logger.info('repo-snapshot: no webServer available, settings UI disabled');
  }
